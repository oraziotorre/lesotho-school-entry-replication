...
use "$dhs_data/dhspooled.dta", clear // using DHS
keep  if age >= 18 & age <= 49
reg lit birthmo_jul c.birthmo_cent 
...
outreg, merge(dv) ctitle("", "< Age 15") nostars noautosumm se
use "$dhs_data/dhspooled.dta", clear // using DHS data
reg spousedu birthmo_jul c.birthmo_cent i.birthmo_jul#c.birthmo_cent i.age i.year if age >= 20 & age <= 49, r 
outreg2 using "$output/tables/marriage.doc", se keep (birthmo_jul) dec(3) append ctitle(DHS, Spouse edu) nocons nonotes
margins if birthmo <=6, at(birthmo_cent == 0) post
...
outreg2 using "$output/tables/health_by_gender.doc", se keep (birthmo_jul) dec(3) append ctitle(Female, K <20) nocons nonotes
margins if birthmo <=6, at(birthmo_cent == 0) post
outreg, merge(dv) ctitle(""," K <20") nostars noautosumm se
reg agefirstkid20 birthmo_jul c.birthmo_cent i.birthmo_jul#c.birthmo_cent i.age i.year if fem == 0 & age >= 20, r 
outreg2 using "$output/tables/health_by_gender.doc", se keep (birthmo_jul) dec(3) append ctitle(Male, K <20) nocons nonotes
margins if birthmo <=6, at(birthmo_cent == 0) post
outreg, merge(dv) ctitle("","K <20") nostars noautosumm se
reg agefirstmarr20 birthmo_jul c.birthmo_cent i.birthmo_jul#c.birthmo_cent i.age i.year if fem == 1, r 
outreg2 using "$output/tables/health_by_gender.doc", se keep (birthmo_jul) dec(3) append ctitle(Female, M <20) nocons nonotes
margins if birthmo <=6, at(birthmo_cent == 0) post
outreg, merge(dv) ctitle("","M <20") nostars noautosumm se
reg agefirstmarr20 birthmo_jul c.birthmo_cent i.birthmo_jul#c.birthmo_cent i.age i.year if fem == 0, r 
outreg2 using "$output/tables/health_by_gender.doc", se keep (birthmo_jul) dec(3) append ctitle(Male, M <20) nocons nonotes
margins if birthmo <=6, at(birthmo_cent == 0) post
outreg, merge(dv) ctitle("","M <20") nostars noautosumm se
...
outreg, clear(dv)
drop if hivpos == . | edu== .
reg edu birthmo_jul c.birthmo_cent i.birthmo_jul#c.birthmo_cent i.age i.year i.fem if age >= 18 & age <= 49, r 
outreg2 using "$output/tables/hiv_by_gender.doc", se keep (birthmo_jul) dec(3) append ctitle(Both genders, Schooling) nocons nonotes
margins if birthmo <=6, at(birthmo_cent == 0) post
...
margins if birthmo <=6, at(birthmo_cent == 0) post
outreg, merge(dv) ctitle("","Schooling") nostars noautosumm se
reg hivpos birthmo_jul c.birthmo_cent i.birthmo_jul#c.birthmo_cent i.age i.year i.fem if age >= 18 & age <= 49, r 
outreg2 using "$output/tables/hiv_by_gender.doc", se keep (birthmo_jul) dec(3) append ctitle(Both genders, HIV) nocons nonotes
margins if birthmo <=6, at(birthmo_cent == 0) post
...
margins, post
outreg, merge(dv) ctitle("","WI") nostars noautosumm se
reg spousedu eduyrs c.birthmo_cent i.age i.year, r 
outreg2 using "$output/tables/decomposition.doc", se keep (eduyrs) dec(3) append ctitle(DHS, Spousal educ) nocons nonotes
margins, post
outreg, merge(dv) ctitle("","Spouse edu") nostars noautosumm se
