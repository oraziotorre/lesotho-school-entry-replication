...
We have been able to replicate 4 different figures and one table.
...
- `LesothoSchoolEntryReplication.jl`  
  Defines the main Julia module of the package. This file includes all source files (`utils.jl`, `figures.jl`, `tables.jl`, and `run_all.jl`) into a single package namespace.

In addition, the package includes a dedicated testing suite:

- `test/runtests.jl`  
  Contains the unit tests implemented with `TestItemRunner.jl`. The tests validate the complete figure/table generation pipeline using synthetic mock datasets.
